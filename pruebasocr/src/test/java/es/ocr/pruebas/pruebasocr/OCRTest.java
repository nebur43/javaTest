...
//			LOGGER.debug("procesando: {}", file.getAbsolutePath());
			...
			if (file.getName().endsWith(".pdf")) {
				...
				txt = PDFUtils.extractText(pdfContent);
	        	cifs = cifAnalayzer.getCifs(txt);
	        	if (!cifs.isEmpty()) {
	        		LOGGER.info("{} | PDF_TXT | CIFS:{} | {}", file.getName(), cifs.size() , cifs);
	        	} else {
	        		List<BufferedImage> images = PDFUtils.extraerImagenes(pdfContent);
	        		if (images.isEmpty()) {
	        		...
	        		cifs = cifAnalayzer.getCifs(txt);
	        		LOGGER.info("{} | PDF_IMAGEN | CIFS:{} | {}", file.getName(), cifs.size(), cifs );
	        	}
				
			} else {
				byte[] jpgContent = EcontratoUtils.readImageFile(file.getAbsolutePath());
	            txt = instance.doOCR(ImageIO.read(new ByteArrayInputStream(jpgContent)));
	            cifs = cifAnalayzer.getCifs(txt);
	            LOGGER.info("{} | IMAGEN | CIFS:{} | {}", file.getName(), cifs.size(), cifs);
			}
